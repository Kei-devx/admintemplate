$(function(){
    ...
    $('#launch').on('click', function(){
        $('#basicExampleModal').modal('show') //popup modal when btn clicked
        $('#modalTitle').text('Add') //set value blank to all input
        $('#title').val('')
        $('#desc').val('')
        $('#image').val('')
    })
...
function editarticle(btnEdit){
    ...
    let imageURL = $(btnEdit).parent().siblings().eq(3).find('img').attr('src')
    console.log(imageURL);
    $('#aaid').val($(btnEdit).attr('data-id'))
    $('#title').val(title)
    $('#desc').val(desc)
    $('#image').val(imageURL)
}
